...
const sq = (x, y, sigma, alpha) => {
    const m = x.length + 1;
    const n = y.length + 1;
    const arr = Array.from({ length: x.length + 1 }, () => Array.from({ length: y.length + 1 }, () => Infinity));
    // for empty strings
    for (let i = 0; i < m; i++)
        arr[i][0] = i * sigma;
    for (let j = 0; j < m; j++)
    ...
    for (let j = 1; j < n; j++) {
        for (let i = 1; i < m; i++) {
            arr[i][j] = Math.min(alpha[x[i - 1]][y[j - 1]] + arr[i - 1][j - 1], sigma + arr[i - 1][j], sigma + arr[i][j - 1]);
        }
    }
    return arr;
};
