...
class MinHeap {
  ...
  decreaseKey = (key: string | number, newVal: number) => {
  ...
  increaseKey = (key: string | number, newVal: number) => {
    // check whether this key belongs to this heap
    if (!this.contains(key)) return false;
    //   get idx of this key
    let idx = this.idxs[key];
  ...
  updateKey = (key: string | number, newVal: number) => {
    // check whether this key belongs to this heap
    if (!this.contains(key)) return false;
    //   get idx of this key
    let idx = this.idxs[key];
