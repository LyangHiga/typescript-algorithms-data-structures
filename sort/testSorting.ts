import sortAndCount from "../divide-and-conquer/countingInversions";
...
const testArr = (a: number[], b: number[]): boolean => {
...
type sorting = (arr: number[]) => number[] | [number[], number];

const performanceTest = (sort: sorting): number => {
...
const test1 = (): void => {
  ...
  const tArr = data.sort((a: number, b: number) => a - b);
...
const test2 = (): void => {
  ...
  pTimes.sort(
    (a: { n: string; t: number }, b: { n: string; t: number }) => a.t - b.t
  );
  console.log(pTimes);
};
